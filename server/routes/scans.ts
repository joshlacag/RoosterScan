...
export const getScans: RequestHandler = async (req, res) => {
  try {
    ...
    // Map database fields to frontend format (lightweight version)
    const mappedScans = (scans || []).map(scan => ({
      id: scan.id,
      roosterId: scan.rooster_id,
      userId: scan.user_id,
      sessionName: scan.session_name,
      scanType: scan.scan_type || 'pose_analysis',
      duration: scan.duration_seconds,
      videoUrl: null, // Not loaded for performance
      thumbnailUrl: null, // Not loaded for performance
      status: scan.status || 'completed',
      poseData: null, // Not loaded for performance - fetch individually if needed
      injuryDetections: scan.injury_detections || [],
      analysisConfidence: scan.analysis_confidence || 0,
      fps: null, // Not loaded for performance
      resolution: null, // Not loaded for performance
      cameraSettings: null, // Not loaded for performance
      ...
      results: { posture: "normal", wing: "normal", legs: "normal", movement: "normal" },
      overlay: false,
      injuries: Array.isArray(scan.injury_detections) ? scan.injury_detections.map((inj: any) => inj.type || 'Unknown') : [],
      severity: (() => {
        const hasInjuries = Array.isArray(scan.injury_detections) && scan.injury_detections.length > 0;
        if (!hasInjuries) return 'low' as "low" | "medium" | "high";
        
        const confidence = scan.analysis_confidence || 0;
        return confidence > 0.8 ? 'high' as "low" | "medium" | "high" : 
               confidence > 0.6 ? 'medium' as "low" | "medium" | "high" : 
