  ...
  const saveScanToDatabase = async (data: PoseResult) => {
    try {
      ...
      const scanData = {
        ...
        duration: Math.round((data.pose_detection?.processing_time || data.processing_time_estimate || 2500) / 1000)
      };
    ...
    try {
      ...
      const scanData = {
        ...
        injuries: (() => {
          ...
          // Check injury classification results
          if (result.injury_classification?.classification && result.injury_classification.classification !== 'healthy') {
            injuries.push(result.injury_classification.classification);
          }
          
          // Check combined analysis for specific findings
          if (result.combined_analysis?.health_assessment && result.combined_analysis.health_assessment !== 'healthy') {
            injuries.push(result.combined_analysis.health_assessment);
          }
          
          // Check for specific injury types from AI output
          if (result.injury_classification?.predictions) {
            Object.entries(result.injury_classification.predictions).forEach(([injury, confidence]) => {
              if (injury !== 'healthy' && typeof confidence === 'number' && confidence > 0.5) {
                injuries.push(injury.replace('_', ' '));
              }
            });
          }
          
          return [...new Set(injuries)]; // Remove duplicates
        })(),
        severity: (() => {
          const injuries = [];
          if (result.injury_classification?.classification && result.injury_classification.classification !== 'healthy') {
          ...
          if (injuries.length === 0) return 'low' as const;
          
          const maxConfidence = result.combined_analysis?.combined_confidence || 
                               result.injury_classification?.confidence || 
                               result.pose_detection?.pose_confidence || 0;
                ...
                <div className="space-y-8">
                          ...
                          <div className="flex items-start gap-3 sm:gap-4 flex-1">
                            ...
                            <div className={`flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center ${
                              (() => {
                                ...
                                const hasIssues = (result.combined_analysis?.specific_findings && result.combined_analysis.specific_findings.length > 0) ||
                                                 (result.injury_analysis?.detected_issues && result.injury_analysis.detected_issues.length > 0);
                                
                                const isHighRisk = result.combined_analysis?.health_assessment === 'bumblefoot' || 
                                                 result.injury_analysis?.risk_level === 'high' ||
                                                 hasIssues;
                                
                                return isHighRisk ? 'bg-red-500' : 'bg-primary';
                              })()
                            }`}>
                              {(() => {
                                // Check if there are any detected issues
                            ...
                            <div className="flex-1 min-w-0">
                              <div className={`inline-flex items-center gap-2 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs font-bold mb-2 ${
                                ...
                                {(() => {
                                  ...
                                  return isHighRisk ? 'INJURY DETECTED' : 'HEALTHY';
                                })()}
                              </div>
                              ...
                              <p className="text-xs sm:text-sm text-muted-foreground">
                                {(() => {
                                  // Check if there are any detected issues first
                                  const hasIssues = (result.combined_analysis?.specific_findings && result.combined_analysis.specific_findings.length > 0) ||
                                                   (result.injury_analysis?.detected_issues && result.injury_analysis.detected_issues.length > 0);
                                  
                                  if (result.combined_analysis?.health_assessment === 'bumblefoot') {
                                    return 'Immediate veterinary attention recommended';
                                  } else if (hasIssues) {
                                    return 'Review detected issues and recommendations below';
                                  } else if (result.combined_analysis?.health_assessment === 'healthy' || 
                                           result.injury_analysis?.risk_level === 'low') {
                                    return 'No significant health concerns identified';
                                  } else {
                  ...
                  {((result.combined_analysis?.recommendations && result.combined_analysis.recommendations.length > 0) ||
                    ...
                    <div>
                      ...
                      {/* Treatment Guide - Only for serious conditions */}
                      {(() => {
                        // Only show treatment guide for specific serious conditions, not minor injuries
                        const healthStatus = result.combined_analysis?.health_assessment;
                        const detectedIssues = result.combined_analysis?.specific_findings || result.injury_analysis?.detected_issues || [];
                        
                        // Check if this is a serious condition that needs a full treatment protocol
                        const isSeriousCondition = healthStatus === 'bumblefoot' || 
                                                 detectedIssues.some(issue => 
                                                   issue.toLowerCase().includes('bumblefoot') ||
                                                   issue.toLowerCase().includes('wing') ||
                                                   issue.toLowerCase().includes('fracture') ||
                                                   issue.toLowerCase().includes('feather loss')
                                                 );
                        
                        if (!isSeriousCondition) return null;
